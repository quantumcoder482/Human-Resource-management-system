angular.module('App').factory("services", function($http) {
  ...
  obj.getSalesReportBooking = function(sdate, edate, type){
     // alert(JSON.stringify(sdate, null, 4));
    return $http.get(serviceBase + 'get_Sales_Report_Booking?sdate=' + sdate + ' && edate='+ edate +'&& type='+ type);
  }
  
  obj.getDaySalesReportBooking = function(theday){
	   // alert(JSON.stringify(sdate, null, 4));
    return $http.get(serviceBase + 'get_Day_Sales_Report_Booking?theday=' + theday);
  }
 ...
 // Product Ingredient Consumption Transaction
  ...
  obj.updateTemporary = function (id, emp) {
    return $http.post(serviceBase + 'updateTemporary', {id:id, emp:emp}).then(function (status) {
        return status.data;
    });
  };
  ...
  obj.updateTempPayment = function (id, emp) {
    return $http.post(serviceBase + 'updateTempPayment', {id:id, emp:emp}).then(function (status) {
        return status.data;
    });
  };

  obj.updateTempPayState =function (id, state) {
    return $http.post(serviceBase + 'updateTempPayState', {id:id, state:state}).then(function (status) {
      return status.data;
    });
  };
  ...
  obj.updateVendor = function (id, vend) {
    return $http.post(serviceBase + 'updateVendor', {id:id, vend:vend}).then(function (status) {
        return status.data;
    });
  };
  ...
  obj.updateVendPayState =function (id, state, pay_date) {
    return $http.post(serviceBase + 'updateVendPayState', {id:id, state:state, pay_date:pay_date}).then(function (status) {
      return status.data;
    });
  };

  obj.updateVendorPay = function (id, vend) {
    return $http.post(serviceBase + 'updateVendorPay', {id:id, vend:vend}).then(function (status) {
        return status.data;
    });
  };
