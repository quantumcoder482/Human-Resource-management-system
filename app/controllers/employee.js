var app=angular.module('App').controller('EmployeeController', 
function($rootScope, $scope, $http, $mdToast, $cookies, $mdDialog, $route, services,$routeParams){
  if($cookies.session_uid == 'null' || $cookies.session_uid == null){ $scope.Expire_Session(); }
    ...
    self.add = $routeParams.add;
	  $rootScope.pagetitle = 'Employee';
	
	
    root.toolbar_menu = { title: 'Add Employee' }
...
function EmployeeControllerDialog($scope, $mdDialog, services, $mdToast, $route, employee) {
  ...
  if (isNew) {
    original = { name : null, sex:null, contact:'',email:'', address:'', aadhaar:'', parent_f:'',parent_m:'',position:'', join:'', salary:'', incentives:'', blood:'', active:'', leave:'' };
