var app=angular.module('App').controller('BookingReportController', 
function($rootScope, $scope, $http, $mdToast, $cookies, $mdDialog, $route, services){
  if($cookies.session_uid == 'null' || $cookies.session_uid == null){ $scope.Expire_Session(); }
	...
	$rootScope.pagetitle = 'Booking Report';
	var self             = $scope;
	var root             = $rootScope;	
  self.loading         = true;
	var to_report = {start_date : toISOLocal(new Date()).split("T")[0], end_date : toISOLocal(new Date()).split("T")[0], date:toISOLocal(new Date()).split("T")[0], month:toISOLocal(new Date()).split("T")[0].slice(0,7), year:toISOLocal(new Date()).split("T")[0].slice(0,4), type : 5};
  ...
  self.kindChange = function(e){
	...
	var daydata = [];
	services.getSalesReportBooking(toISOLocal(new Date()).split("T")[0],toISOLocal(new Date()).split("T")[0], 0).then(function(data){
	...
	self.get_sales_report_booking = function(dates) {
		self.daytable = false;
    switch(dates.kind) {
      case 1:
          services.getSalesReportBooking(dates.date, dates.date, dates.type).then(function(data){
            if (data.data[0].total > 0){
              if (!data.data[0].hasOwnProperty('type')) {
                data.data[0].type = 'All';
              }else {
              ...
              data.data[0].total = Number(data.data[0].total.toFixed(2));
              self.report = data.data;
              services.getDaySalesReportBooking(dates.date).then(function(data){
                if(data.data != ''){
                  var day_report;
                  for (var i=0; i<data.data.length; i++){
                    var itemarr = data.data[i].items.split(',');
          ...
          break;
      case 2:
          services.getSalesReportBooking(dates.month+'-01', dates.month+'-31', dates.type).then(function(data){
            if (data.data[0].total > 0){
              if (!data.data[0].hasOwnProperty('type')) {
                data.data[0].type = 'All';
              }else {
          ...
          break;
      case 3:
          services.getSalesReportBooking(dates.year+'-01-01', dates.year+'-12-31', dates.type).then(function(data){
            if (data.data[0].total > 0){
              if (!data.data[0].hasOwnProperty('type')) {
                data.data[0].type = 'All';
              }else {
          ...
          break;
      case 4:
          services.getSalesReportBooking(dates.start_date, dates.end_date, dates.type).then(function(data){
            if (data.data[0].total > 0){
              if (!data.data[0].hasOwnProperty('type')) {
                data.data[0].type = 'All';
              }else {
              ...
              if(dates.start_date == dates.end_date){
                services.getDaySalesReportBooking(dates.start_date).then(function(data){
                  if(data.data != ''){
                    var day_report;
                    for (var i=0; i<data.data.length; i++){
                      var itemarr = data.data[i].items.split(',');
