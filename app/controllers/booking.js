...
function($rootScope, $scope, $http, $mdToast, $cookies, $mdDialog, $route, services){
  if($cookies.session_uid == 'null' || $cookies.session_uid == null){ $scope.Expire_Session(); }
    ...
    original = { items : [], booking_date: toISOLocal(new Date()).split("T")[0], quantity:[], prices:[],item_names:[],cgst:[],sgst:[],user_id:0 ,contact_name:null,contact_number:null,contact_address:null,bill_amount:0,payable_amount:0,customer_amount:null,order_cgst:0,order_sgst:0,order_comment:null,discount:0,type:1, gst_price:[], totalpaid:0, dispute:0};
	...
	self.AddBookingDate = function(){
		if(!self.bookingdate1) {self.bookingdate1 = true; return;}
		if(self.bookingdate1 && !self.bookingdate2) {self.bookingdate2 = true; return;}
		if(self.bookingdate2 && !self.bookingdate3) {self.bookingdate3 = true; return;}
		if(self.bookingdate3 && !self.bookingdate4) {self.bookingdate4 = true; return;}
	}
	...
	services.ActiveCategories().then(function(data){
	  ...
	  services.getMenulist().then(function(data){
	    ...
	    for (var i=0; i<data.data.length; i++){
	      ...
	      list.push({id: data.data[i].id, name: data.data[i].name, info: info, price: data.data[i].payable_amount, item_ids: data.data[i].items, item_counts:data.data[i].quantity});
	      info = '';
	    }
